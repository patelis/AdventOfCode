...
# Create binary df the tidy way
df <- data %>% tidyr::separate("string", sep = 1:11, into = paste0("bit", 1:12), convert = TRUE) %>% as_tibble()

# Create binary df with base R
l <- strsplit(data$string, split = NULL)
df_base <- do.call(rbind.data.frame, l)
names(df_base) <- paste0("bit", 1:12)
df_base <- as.data.frame(sapply(df_base, as.integer))
...
common_filter <- function(data, n = 1, opposite = FALSE) {
  ...
  if (n <= length(data) & nrow(data) > 1) {
    ...
    data_filtered <- data %>% dplyr::filter(.data[[column]] == value_to_filter)
    ...
    data %>% 
      convert_binary_df_to_decimal_num()
...
# base R solution - exactly same except for the filter which is replaced with a eval + parse + string solution

common_filter_base <- function(data, n = 1, opposite = FALSE) {
    ...
    string <- paste0("data[data[['bit", n, "']] == value_to_filter, ]")
    data_filtered <- eval(parse(text = string))

    common_filter_base(data_filtered, n + 1, opposite = opposite)
...
oxygen_base <- common_filter_base(df_base)
  
co2_base <- common_filter_base(df_base, opposite = TRUE)

oxygen_base * co2_base
