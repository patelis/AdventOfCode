...
for (i in 1:bingo_players) {
  ...
  bingo_boards[[i]] <- data[(1 + (i - 1) * 5):(5 + (i - 1) * 5)]
  
  bingo_boards[[i]] <- bingo_boards[[i]] |> 
    unlist() |> 
    strsplit(split = " ")

  bingo_boards[[i]] <- lapply(1:5, FUN = (\(x) bingo_boards[[i]][[x]][bingo_boards[[i]][[x]] != ""])) |>
    unlist() |>
    as.numeric()|>
...
first_five <- numbers_called_out[1:5]
remaining_numbers <- numbers_called_out[-(1:5)]
...
while (!loss_flag) {
  ...
  col_win <- all(col_win_vec | row_win_vec)
